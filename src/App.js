        ...
        for (let i = 0; i < data.daily.length; i++) {
            dailyList.push(<div className='next-weather' key={data.daily[i].dt}>
                ...
                <div className='description-block'>
                    ...
                    {/*<div className="inline">*/}
                    {/*    <div>Утром {Math.trunc(+data.daily[i].temp.morn - 273.15)}°</div>*/}
                    {/*    <div>Днём {Math.trunc(+data.daily[i].temp.day - 273.15)}°</div>*/}
                    {/*    <div>Вечером {Math.trunc(+data.daily[i].temp.eve - 273.15)}°</div>*/}
                    {/*    <div>Ночью {Math.trunc(+data.daily[i].temp.night - 273.15)}°</div>*/}
                    {/*</div>*/}
                    {/*<div className="inline">*/}
                    {/*    <div className='wind'>*/}
                    {/*        Скорость ветра {Math.round(+data.daily[i].wind_speed * 10) / 10} м/c*/}
                    {/*        <img src={vector} alt="" style={{transform: `rotate(${+data.daily[i].wind_deg}deg)`}}/>*/}
                    {/*    </div>*/}
                    {/*    <div>Порывы ветра до {Math.round(+data.daily[i].wind_gust * 10) / 10} м/с</div>*/}
                    {/*</div>*/}
                    {/*<div className="inline">*/}
                    {/*    <div>Облачность {data.daily[i].clouds}%</div>*/}
                    {/*    <div>Давление {Math.round(+data.daily[i].pressure / 1.33)} мм.рт.ст.</div>*/}
                    {/*    <div>Влажность {data.daily[i].humidity}%</div>*/}
                    {/*</div>*/}
                    {/*<div className="inline">*/}
                    {/*    <div>Восход {new Date(+data.daily[i].sunrise * 1000).toLocaleTimeString()}</div>*/}
                    {/*    <div>Закат {new Date(+data.daily[i].sunset * 1000).toLocaleTimeString()}</div>*/}
                    {/*</div>*/}
                </div>
            </div>)
        }
                    ...
                    <div>
                        <div className='inline'>
                            ...
                            <div className='wind description'>
                                ...
                                <img src={vector} alt="" style={{transform: `rotate(${+data.current.wind_deg}deg)`}}/>
                            </div>
                        </div>
                        <div className='inline'>
                            <div>
